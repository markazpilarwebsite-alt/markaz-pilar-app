...
const emptyForm = {
  full_name: '', email: '', phone: '', university: '', major: '', division: '',
  pesantren_id: '', batch_tahun: '', provinsi_domisili: '', kota_domisili: '', status: 'active'
}
...
export default function KakakPengajarPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    const { data } = await supabase.from('kakak_pengajar').select('*, pesantren(name)').order('created_at', { ascending: false })
    setList(data || [])
    setLoading(false)
  }
  ...
  const openEdit = (item) => {
    ...
    setForm({
      ...
      university: item.university || '', major: item.major || '', division: item.division || '',
      pesantren_id: item.pesantren_id || '', batch_tahun: item.batch_tahun || '',
      provinsi_domisili: item.provinsi_domisili || '', kota_domisili: item.kota_domisili || '',
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    const payload = { ...form, pesantren_id: form.pesantren_id || null, batch_tahun: form.batch_tahun || null }
    if (editData) {
      await supabase.from('kakak_pengajar').update(payload).eq('id', editData.id)
    } else {
      await supabase.from('kakak_pengajar').insert(payload)
    }
  ...
  const handleDelete = async (id) => {
    ...
    await supabase.from('kakak_pengajar').delete().eq('id', id); fetchData()
  }
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-x-auto shadow-sm">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  {[['full_name','Nama'],['pesantren_id','Pesantren'],['batch_tahun','Batch'],['university','Universitas'],['major','Jurusan'],['division','Divisi'],['provinsi_domisili','Domisili'],['status','Status']].map(([key, label]) => (
              ...
              <tbody>
                {sortedList.map((item, i) => (
                  <tr key={item.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-4 py-3 text-gray-600">{item.provinsi_domisili || '-'}</td>
                    <td className="px-4 py-3">
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={form.university} onChange={e => setForm({...form, university: e.target.value})} placeholder="Universitas Indonesia" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Jurusan</label>
                <input value={form.major} onChange={e => setForm({...form, major: e.target.value})} placeholder="Teknik Informatika" className={inputClass} />
              </div>
              <div>
                <label className={labelClass}>Divisi</label>
                <input value={form.division} onChange={e => setForm({...form, division: e.target.value})} placeholder="Program" className={inputClass} />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.provinsi_domisili} onChange={e => setForm({...form, provinsi_domisili: e.target.value})} className={inputClass}>
                ...
                <div>
                  ...
                  <input value={form.kota_domisili} onChange={e => setForm({...form, kota_domisili: e.target.value})} placeholder="Kota/Kab..." className={inputClass} />
                </div>
              </div>
              <div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                ['Universitas', detailData.university],
                ['Jurusan', detailData.major],
                ['Divisi', detailData.division],
                ['No. HP', detailData.phone],
                ['Provinsi Domisili', detailData.provinsi_domisili],
                ['Kota/Kab Domisili', detailData.kota_domisili],
